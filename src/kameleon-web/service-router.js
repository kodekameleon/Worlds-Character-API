...
export async function main(event) {
  try {
    ...
    // Fix the query parameters so that object parameters are objects
    fixParameters(event.queryStringParameters);
...
function fixParameters(queryParams) {
  Object.keys(queryParams).forEach(key => {
    if (pathToObject(queryParams, key, queryParams[key])) {
      delete queryParams[key];
    }
  });
}

function pathToObject(obj, path, value, index = 0) {
  if (typeof path === "string") {
    if (path.includes(".")) {
      return pathToObject(obj, path.split("."), value, 0);
    } else {
      return;
    }
    ...
    obj[path[index]] = pathToObject(obj[path[index]] || {}, path, value, index + 1);
    return obj;
  }
}
